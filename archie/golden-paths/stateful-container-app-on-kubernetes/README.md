# Stateful Container App on Kubernetes
...
2. `cd dynamodb` — `tofu init -backend-config=...` then `tofu apply`
3. `cd vpc` — `tofu init -backend-config=...` then `tofu apply`
4. `cd eks` — `tofu init -backend-config=...` then `tofu apply`
5. `cd workload` — `tofu init -backend-config=...` then `tofu apply`
